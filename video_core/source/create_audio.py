...
INDEX_TTS2_APP_ID = "1965684535247650818"
...
def _build_node_info_list(
    *,
    ...
    reference_audio_text = str(reference_audio_file or "").strip()
    if reference_audio_text:
        if app_id_text == INDEX_TTS2_APP_ID:
            return [
                {"nodeId": "9", "fieldName": "audio", "fieldValue": reference_audio_text, "description": "上传克隆音频"},
                {"nodeId": "6", "fieldName": "text", "fieldValue": f"{word}", "description": "上传语音文本"},
                {"nodeId": "17", "fieldName": "text", "fieldValue": f"{emotion}", "description": "情感描述"},
            ]
        return [
            {"nodeId": "4", "fieldName": "audio", "fieldValue": reference_audio_text, "description": "reference_audio"},
            {"nodeId": "7", "fieldName": "text", "fieldValue": f"{word}", "description": "script_text"},
        ]
    if app_id_text == INDEX_TTS2_APP_ID:
        return [
            {"nodeId": "6", "fieldName": "text", "fieldValue": f"{word}", "description": "上传语音文本"},
...
def submit_audio_task(
    *,
    ...
    url = f"{str(runninghub_common.BASE_URL).rstrip('/')}/openapi/v2/run/ai-app/{app_id_text}"
...
def query_task_status(api_key, task_id):
    url = f"{str(runninghub_common.BASE_URL).rstrip('/')}/openapi/v2/query"
    headers = {
