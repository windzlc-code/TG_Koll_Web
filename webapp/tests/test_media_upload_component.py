...
class MediaUploadComponentContractTests(unittest.TestCase):
    ...
    def test_hotspot_cards_open_media_and_source_without_detail_modal(self):
        ...
        self.assertIn('const videoSrc = isVideo ? adminWorkspaceUrl(previewUrl) : "";', preview)
        self.assertNotIn("const videoSrc = isVideo && isVideoMediaUrl(sourceUrl)", preview)
    ...
    def test_hotspot_editor_reuses_public_modal_and_complete_media_editor(self):
        ...
        self.assertIn("data-persona-hot-rewrite-instruction", modal_renderer)
        self.assertIn("改写提示词（选填）", modal_renderer)
        self.assertIn("可继续输入新提示再调整", modal_renderer)
        self.assertIn("按提示改写", self.script)
        rewrite_fn = self.script[
            self.script.index("async function rewritePersonaHotEditorContent"):
        ...
        self.assertIn("AI 改写", modal_renderer)
        self.assertIn("AI 改写中", self.script)
        self.assertIn("按提示改写中", self.script)
        self.assertNotIn("再次按人设改写", self.script)
        self.assertIn("personaHotRewriteActionLabel", modal_renderer)
        self.assertIn("instruction,", self.script[self.script.index("async function rewritePersonaHotEditorContent"):self.script.index("\nfunction resizePersonaHotEditorContent")])
    ...
    def test_ai_generation_reuses_horizontal_image_upload_without_preview_tile(self):
        ...
        self.assertIn('class="account-pool-add-button upload-zone-mobile-picker upload-add-chip"', self.script)
        self.assertIn("label.account-pool-add-button.upload-add-chip {", self.styles)
        self.assertIn("border-radius: 999px;", self.styles.split("label.account-pool-add-button.upload-add-chip {", 1)[1].split("}", 1)[0])
        self.assertIn("background: transparent;", self.styles.split("label.account-pool-add-button.upload-add-chip {", 1)[1].split("}", 1)[0])
    ...
    def test_ai_upload_lives_inside_task_preview_and_appends_generated_images(self):
        ...
        self.assertIn("const hasDraftMediaPreview = Boolean(postMediaItems.length);", inline)
        self.assertIn("hideEmpty: hasDraftMediaPreview", inline)
        self.assertNotIn("hideEmpty: Boolean(postMediaItems.length || aiUploadSelectsModify)", inline)
        self.assertNotIn("renderPersonaMediaOperationTabs(", inline)
        self.assertNotIn("自定义上传", inline)
        self.assertNotIn('source: "posts"', inline)
        self.assertIn("renderPersonaImageStylePicker(persona, post", inline)
    ...
    def test_edit_draft_ai_upload_selection_is_the_media_modify_state(self):
        ...
        self.assertIn('modifyAttribute: ""', upload_renderer)
        self.assertNotIn("!selectionControlsModify ? `<button type=\"button\"", upload_renderer)
        ...
        self.assertIn("setPersonaTaskMediaModifySource(mediaKey)", self.script)
