...
class AutomationPlanFrontendContractTests(unittest.TestCase):
    ...
    def test_plan_reuses_existing_task_configuration_instead_of_duplicate_fields(self):
        ...
        self.assertIn("buildPersonaThreadsTaskPayload(kind, platform)", CONSOLE_JS)
        self.assertIn("cloneAutomationPlanPayload(buildPersonaThreadsTaskPayload(kind, platform))", CONSOLE_JS)
        ...
        self.assertIn("const options = automationPlanTaskOptions(platform);", picker_options)
        self.assertNotIn('["automation_mode"', picker)
        self.assertIn('["threads_reply_comment", "threads_reply_hot", "threads_warmup", "instagram_warmup"].includes(taskType)', picker)
        self.assertIn('isCurrentTaskType ? null : { initialStep: automationPlanStepForTask(taskType) }', picker)
        self.assertNotIn('item.taskType = "threads_warmup"', picker)
        self.assertIn("selectedTaskType === taskType", picker)
        self.assertIn("openAutomationPlanNormalPublishConfigurator(index)", picker)
        self.assertIn("automation-plan-task-control.is-empty", CONSOLE_CSS)
        self.assertIn(".automation-plan-task-control.is-empty .automation-plan-task-picker", CONSOLE_CSS)
        self.assertIn("appearance: none;", CONSOLE_CSS)
        self.assertIn(".automation-plan-task-option:hover", CONSOLE_CSS)
        self.assertIn(".automation-plan-task-option:focus-visible", CONSOLE_CSS)
    ...
    def test_plan_detail_overviews_only_show_task_strategy(self):
        ...
        self.assertIn("automationPlanStrategySummary", strategy_fields)
        self.assertIn('["browse_limit", "scroll_times", "like_limit", "max_comments"]', strategy_fields)
        self.assertIn("automationPlanStrategyFieldLabel(key)", strategy_fields)
        self.assertNotIn("Object.entries(params)", strategy_fields)
        self.assertNotIn("persona_context", strategy_fields)
        self.assertNotIn("persona_personality", strategy_fields)
        self.assertNotIn("like_chance", strategy_fields)
        self.assertNotIn("comment_chance", strategy_fields)
        self.assertNotIn("预约时间", form_rows)
        self.assertNotIn("执行时间", form_rows)
        self.assertNotIn("任务内容", form_rows)
        self.assertNotIn("automationPlanTaskContent(task)", form_rows)
    ...
    def test_plan_uses_bound_account_notice_and_confirms_execution_platform_before_submit(self):
        current = function_source("currentAutomationPlanDraft", "automationReservationLabel")
        panel = function_source("renderAutomationTaskPlanPanel", "normalizePublishContentSource")
        picker = function_source("choosePublishPlatformAccount", "publishPlatformLabel")
        submit = function_source("submitAutomationPlan", "cancelAutomationPlan")
        ...
        self.assertIn('modalKey: "automation-plan-platform-picker"', submit)
        self.assertIn("persistSelection: false", submit)
        self.assertIn("if (!account) return;", submit)
        self.assertLess(submit.index("choosePublishPlatformAccount"), submit.index('/api/persona_dashboard/automation/plans'))
        self.assertIn("account_id: account.id", submit)
        self.assertIn('platform: account.platform || "threads"', submit)
