...
class PersonaDashboardApiTests(unittest.TestCase):
    ...
    def test_public_full_refresh_defaults_to_http_first_source(self):
        self._write_archives()
        with mock.patch.object(
            server,
    ...
    def test_background_refresh_keeps_http_first_source(self):
    ...
    def test_hot_keyword_prompt_keeps_full_generation_spec(self):
        ...
        self.assertIn("候选正文中自然提到一嘴就符合", importer)
        self.assertIn("程序不会用固定词表或兜底代码补齐", importer)
        self.assertIn("normalQueries 正好 10 个", importer)
        self.assertIn("同一内容类型或受众生态", importer)
        self.assertIn("允许与严格垂直有少量自然重叠", importer)
        self.assertNotIn("personaBridgeTerms", importer)
        self.assertIn("不要求关键词逐字包含人设原文", importer)
        self.assertNotIn("逐字提取 4-8 个 personaBridgeTerms", importer)
    ...
    def test_persona_hot_keywords_consume_remaining_batches_then_regenerate(self):
        self._write_archives()
        initial_keywords = [f"keyword-{index}" for index in range(20)]
        regenerated_keywords = [f"fresh-{index}" for index in range(20)]
    ...
    def test_persona_hot_keyword_batch_is_not_consumed_when_collection_fails(self):
        ...
        self.assertEqual(prepared["keywords"], keywords[:10])
    ...
    def test_persona_generation_instruction_uses_native_locale_writing_rules(self):
        ...
        self.assertIn("Every generated post in this batch must stay in the same selected locale", japanese)
