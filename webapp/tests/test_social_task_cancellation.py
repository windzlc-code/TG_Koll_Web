...
class SocialTaskCancellationTests(unittest.TestCase):
    ...
    def test_effective_account_status_combines_login_and_platform_health(self):
        ...
        self.assertEqual(effective({"status": "ready", "health_status": "banned"}), "risk_control")
        self.assertEqual(effective({"status": "cookie_expired", "health_status": "alive"}), "cookie_expired")
        self.assertEqual(effective({"status": "disabled", "health_status": "banned"}), "risk_control")
        self.assertEqual(
            effective(
                {
                    "status": "ready",
    ...
    def test_force_stop_closes_registered_browser_manager(self):
        ...
        control = {"cancel_event": cancel_event, "context": context, "manager": manager}

        with (
            mock.patch.dict(
                social_automation_api._RUNNING_TASK_CONTROLS,
                {"force-manager-task": control},
                clear=True,
            ),
            mock.patch("social_automation.live_browser.stop_live_browser_sessions_for_task") as stop_live_browser,
        ):
            stopped = social_automation_api._force_stop_running_task("force-manager-task")
