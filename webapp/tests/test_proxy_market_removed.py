...
class ProxyMarketRemovalTests(unittest.TestCase):
    ...
    def test_account_proxy_picker_is_the_only_user_purchase_and_selection_surface(self):
        ...
        self.assertIn('本月免费机会领取后不可重复使用', self.console_script)
        self.assertIn('选择后显示 IP、端口和凭据', self.console_script)
        self.assertIn('ownershipType === "owned" ? "已购买" : (proxyId ? "已获取" : "免费")', self.console_script)
        ...
        self.assertIn("account-proxy-purchase-step", self.console_script)
        self.assertIn("account-proxy-mapped-product", self.console_script)
    ...
    def test_backend_enforces_monthly_free_selection_and_masks_unclaimed_details(self):
        ...
        self.assertIn("本月免费代理机会已使用，下月可重新选择", self.system_proxy_pool_source)
        self.assertIn('claim_mode="monthly_free"', self.system_proxy_pool_source)
