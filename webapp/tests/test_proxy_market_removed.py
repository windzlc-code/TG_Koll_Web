...
class ProxyMarketRemovalTests(unittest.TestCase):
    ...
    def test_account_proxy_picker_is_the_only_user_purchase_and_selection_surface(self):
        ...
        self.assertIn('暂不选择', self.console_script)
        self.assertIn('<p>选择后自动加入列表。</p>', self.console_script)
        ...
        self.assertIn("linear-gradient(100deg, #168fbd", self.console_styles)
        ...
        self.assertIn("linear-gradient(100deg, #168fbd 0%, #147db2 30%, #115b91 62%, #102c47 100%)", self.console_styles)
