...
class ProxyMarketRemovalTests(unittest.TestCase):
    ...
    def test_account_proxy_picker_is_the_only_user_purchase_and_selection_surface(self):
        ...
        self.assertIn('>用户选择</button>', self.console_script)
        ...
        self.assertIn('data-account-proxy-filter-menu="country"', self.console_script)
        self.assertIn('data-account-proxy-filter-options="country"', self.console_script)
        ...
        self.assertIn("grid-template-columns: repeat(3, minmax(0, 1fr));", self.console_styles)
        self.assertIn('.account-proxy-type-tabs .account-proxy-clear[aria-pressed="true"]', self.console_styles)
        ...
        self.assertIn("max-height: 156px;", self.console_styles)
