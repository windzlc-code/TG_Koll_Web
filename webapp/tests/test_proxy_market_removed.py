...
class ProxyMarketRemovalTests(unittest.TestCase):
    ...
    def test_account_proxy_picker_is_the_only_user_purchase_and_selection_surface(self):
        ...
        self.assertIn('>可选IP <span class="record-tab-count"', self.console_script)
        self.assertIn('>已获得IP <span class="record-tab-count"', self.console_script)
        ...
        self.assertIn("function resolveAccountProxyChoice", self.console_script)
        self.assertIn("function setAccountProxyPickerNotice", self.console_script)
        self.assertIn("function paintAccountProxySelection", self.console_script)
        self.assertIn("function highlightAccountProxyCard", self.console_script)
        self.assertIn("const highlighted = Boolean(proxyId) && proxyId === highlightId", self.console_script)
        self.assertIn("const bound = Boolean(proxyId) && proxyId === boundId", self.console_script)
        self.assertIn("const action = bound ? \"解除使用\" : \"选择使用\"", self.console_script)
        self.assertIn('data-account-proxy-proxy-id="${esc(proxyId)}"', self.console_script)
        self.assertNotIn("该代理尚未通过真实网络检测，请稍后重试。", self.console_script)
