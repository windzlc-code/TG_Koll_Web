...
class AdminProxyPurchaseFrontendTests(unittest.TestCase):
    ...
    def test_bundle_social_config_is_a_masked_parallel_tab(self):
        ...
        self.assertIn('id="bundleSocialApiBaseUrl" type="url"', runtime_panel)
        self.assertNotIn('id="bundleSocialWebhookUrl"', runtime_panel)
        self.assertIn('id="bundleSocialWebhookSecret"', runtime_panel)
        self.assertIn('id="bundleSocialHomepageOverlay"', runtime_panel)
        self.assertIn("bundleSocialWebhookSecret", self.script)
        self.assertIn("formatBundleSocialStatusMessage", self.script)
        self.assertIn("/api/admin/bundle-social/config/secrets/", self.script)
        self.assertIn("/api/admin/proxy-purchases/provider-credentials/secrets/", self.script)
        self.assertIn("PROVIDER_SECRET_REVEAL_PATHS", self.script)
        self.assertNotIn("已显示完整内容", self.script)
        self.assertNotIn("API Key 已显示，再次点击图标可隐藏", self.script)
        self.assertIn("不会强制刷新", runtime_panel)
        self.assertIn("不会导入历史帖", runtime_panel)
        self.assertNotIn("第三方平台", runtime_panel)
        self.assertIn('"bundleSocialApiKey"', self.script)
        self.assertIn("webhook_secret", self.script)
        self.assertIn("homepage_overlay_enabled", self.script)
