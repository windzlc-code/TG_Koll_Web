...
class ConsoleSessionBoundaryTests(unittest.TestCase):
    ...
    def test_account_proxy_picker_replaces_legacy_edit_checkbox_and_keeps_single_binding(self):
        ...
        selector_click = self._function_source("handleAccountProxySelectorClick")
        selector_complete = self._function_source("completeAccountProxySelectorSelection")
        ...
        self.assertIn("openAccountProxySelectorPage", picker)
        self.assertIn("accountProxyOptionCardsHtml(selectedProxyId", selector_page)
        self.assertIn("commitAccountProxyPickerSelection", selector_complete)
        ...
        self.assertIn("await claimAccountProxyPoolOption", selector_click)
        self.assertIn("await completeAccountProxySelectorSelection(page, claimedProxyId)", selector_click)
        self.assertIn("await completeAccountProxySelectorSelection(page, purchasedProxyId)", selector_click)
        self.assertIn('const originalProxyId = normalizedMode === "create"', selector_open)
        self.assertNotIn("data-account-proxy-picker-save", selector_page)
        self.assertNotIn('<div class="console-modal-actions">', selector_page)
    ...
    def test_account_proxy_selection_uses_an_independent_page_and_shared_entry_card(self):
        ...
        picker_page = self._function_source("renderAccountProxySelectorPage")
        picker_route = self._function_source("syncAccountProxySelectorRoute")

        self.assertIn('data-panel="proxy_selector"', self.markup)
        self.assertIn('id="accountProxySelectorPage"', self.markup)
        ...
        self.assertIn('openAccountProxySelectorPage', picker_open)
        self.assertNotIn('document.createElement("div")', picker_open)
        self.assertIn('accountProxyPoolFiltersHtml("page"', picker_page)
        self.assertIn('accountProxyOptionCardsHtml(selectedProxyId, { scope: "page" })', picker_page)
        self.assertIn('url.searchParams.set("view", "proxy_selector")', picker_route)
        self.assertIn('.console-modal[hidden]', self.styles)
        self.assertIn('.account-proxy-selector-shell', self.styles)
    ...
    def test_account_create_uses_the_full_editor_instead_of_the_legacy_form(self):
        ...
        self.assertIn("填写账号密码", create_entry)
        self.assertIn("官方授权", create_entry)
        self.assertIn("openAccountPoolEditorModal({ ...options, platform })", create_entry)
        self.assertIn("adminWorkspaceUrl", create_entry)
        self.assertIn("/oauth/start", create_entry)
        ...
        self.assertIn("选择 ${esc(platformLabel)} 账号接入方式", create_entry)
    ...
    def test_account_proxy_picker_uses_the_public_pool_without_custom_proxy_forms(self):
        ...
        self.assertIn("accountProxyPoolFiltersHtml", selector_page)
        self.assertIn("loadAccountProxyPickerPool", selector_page)
        self.assertIn("claimAccountProxyPoolOption", selector_click)
        ...
        self.assertIn('initialProxyId === null || initialProxyId === undefined', selector_open)
