...
class ConsoleSessionBoundaryTests(unittest.TestCase):
    ...
    def test_account_proxy_selection_uses_an_independent_page_and_shared_entry_card(self):
        ...
        self.assertIn('免费选择专属代理 IP', picker_entry_copy)
        self.assertIn('本月免费一次，开通后自动加入列表', picker_entry_copy)
        self.assertNotIn('本月可免费选择一次，开通后自动加入你的可选代理列表', picker_entry_copy)
        self.assertIn('选择专属代理 IP', picker_entry_copy)
    ...
    def test_account_create_uses_the_full_editor_instead_of_the_legacy_form(self):
        ...
        self.assertIn('platform === "threads" ? "threads_api" : "google"', method_modal)
        self.assertIn('platform === "threads" ? "Threads 接入" : "Google 快速接入"', method_modal)
        self.assertIn("/api/threads/oauth/start", method_modal)
        self.assertIn("Threads 接入通过官方授权同步可用数据", method_modal)
