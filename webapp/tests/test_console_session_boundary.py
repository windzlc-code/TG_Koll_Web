...
class ConsoleSessionBoundaryTests(unittest.TestCase):
    ...
    def test_open_login_account_actions_preserve_running_task_navigation(self):
        ...
        self.assertNotIn('${renderNetworkIcon()}<span data-account-proxy-label>${esc(proxyLabel)}</span>', actions)
        self.assertIn('const proxyAction = "";', actions)
    ...
    def test_account_proxy_picker_replaces_legacy_edit_checkbox_and_keeps_single_binding(self):
        ...
        self.assertNotIn('data-account-proxy-picker="${esc(accountId)}"', card)
        ...
        self.assertNotIn("renderAccountProxyPickerPanel(account, mode)", editor)
    ...
    def test_account_create_collects_credentials_before_platform_authorization_and_keeps_browser_editor_for_legacy_accounts(self):
        editor = self._function_source("renderAccountEditorForm")
        identity = self._function_source("renderAccountIdentityFields")
        ...
        self.assertNotIn('createSocialTask("open_login"', save)
        self.assertIn("startBundleAccountAuthorization", bundle_modal)
        self.assertNotIn("window.open", bundle_modal)
        self.assertNotIn("openBundleAccountSwitchPopup", self.source)
        self.assertNotIn("openBundleAuthorizationWaitingModal", self.source)
        self.assertIn("授权助手", assistance)
        self.assertIn("data-login-assistance-live", assistance)
        self.assertIn("data-login-assistance-close", assistance)
        self.assertIn("continueAccountLoginSyncUntilSettled", assistance)
        self.assertIn("pollBundleAuthorizationStatus", assistance)
        self.assertNotIn("openBundleAuthorizationPopup", self.source)
        self.assertNotIn('value: "switch-account"', bundle_modal)
        ...
        self.assertIn('editing ? "保存修改" : "保存并授权"', modal)
        self.assertIn("startBundleAccountAuthorization", save)
        self.assertIn("accountId: account.id", save)
