...
class ConsoleSessionBoundaryTests(unittest.TestCase):
    ...
    def test_login_assistance_refreshes_when_verification_field_contract_changes(self):
        ...
        render_key = update_source.split("const renderKey = JSON.stringify([", 1)[1].split("]);", 1)[0]
        ...
        self.assertIn("updateLoginAssistanceDeadline", update_source)
    ...
    def test_open_login_account_actions_preserve_running_task_navigation(self):
        ...
        self.assertIn('renderAccountOpenLoginButtonContent(activeLoginTask, account)', actions)
    ...
    def test_account_and_browser_status_refresh_while_publish_is_running(self):
        ...
        self.assertNotIn("publishAssistanceRestoreTaskId", self.source)
        self.assertIn("isPublishAssistanceOpen()", self._function_source("shouldShowPublishAssistanceRestore"))
        self.assertIn("mobilePublishingTask()", self._function_source("shouldShowPublishAssistanceRestore"))
        self.assertIn("hidePublishAssistanceRestore()", self._function_source("openTaskAssistanceView"))
        self.assertIn("data-restore-publish-assistance", self.source)
        self.assertNotIn('data-login-assistance-stop-close', self._function_source("renderLoginAssistanceAction"))
    ...
    def test_account_pool_copy_controls_only_transfer_username_password_and_totp(self):
        ...
        self.assertNotIn("账号用户名（可选）", identity)
        self.assertNotIn("data-account-platform-username-field", identity)
        self.assertNotIn('id="${prefix}Username"', identity)
    ...
    def test_account_editor_accepts_platform_supported_login_identifiers(self):
        ...
        self.assertNotIn("data-account-platform-username-field", identity)
        self.assertNotIn("账号用户名（可选）", identity)
        self.assertNotIn("用于账号卡显示和登录后的账号校验", identity)
        self.assertNotIn("function refreshAccountLoginIdentifierFields", self.source)
        self.assertNotIn("accountPoolUsername", self.source)
        self.assertNotIn("accountPoolEditUsername", self.source)
        self.assertNotIn('accountPoolDraftValue("username")', self.source)
        self.assertIn("login_username: loginIdentifier", create_save)
        self.assertIn("login_username: loginIdentifier", edit_save)
        self.assertIn("username: loginIdentifier", create_save)
        self.assertIn("username: loginIdentifier", edit_save)
        self.assertIn('payload["login_username"] = str(payload.get("login_username")', runtime)
        self.assertNotIn(".account-create-form [hidden]", self.styles)
    ...
    def test_account_proxy_selection_uses_a_shared_floating_modal_and_entry_card(self):
        picker_entry = self._function_source("renderAccountProxyPickerPanel")
        picker_entry_card = self._function_source("accountProxyEntryCardHtml")
        picker_entry_copy = self._function_source("accountProxyEntryCopy")
        picker_open = self._function_source("openAccountProxyPickerModal")

        self.assertNotIn('data-panel="proxy_selector"', self.markup)
        self.assertNotIn('id="accountProxySelectorPage"', self.markup)
        self.assertIn('accountProxyEntryCardHtml', picker_entry)
        self.assertIn('class="account-proxy-entry-card"', picker_entry_card)
        self.assertIn('data-account-proxy-picker-open', picker_entry_card)
        self.assertIn('title: "代理 IP"', picker_entry_copy)
        self.assertIn("accountProxyBoundLabel(cleanId, poolData, proxySource)", picker_entry_copy)
        self.assertIn(': "未使用代理 IP"', picker_entry_copy)
        self.assertIn("function accountProxyBoundLabel", self.source)
        self.assertIn("function resolveAccountProxyForLabel", self.source)
        self.assertNotIn('按地区和城市选择', picker_entry_copy)
        self.assertNotIn('poolData?.monthly_free?.available !== false', picker_entry_copy)
        self.assertIn('action: "选择代理"', picker_entry_copy)
        self.assertNotIn("免费选择", picker_entry_copy)
        self.assertIn('account-proxy-entry-icon', picker_entry_card)
        self.assertEqual(picker_entry_card.count('renderNetworkIcon()'), 2)
        self.assertNotIn('renderShoppingBagIcon()', picker_entry_card)
        self.assertNotIn('account-residential-proxy-head', picker_entry)
        self.assertNotIn('data-account-proxy-selection-summary', picker_entry)
        entry_card_style = self.styles[self.styles.index('.account-proxy-entry-card {'):]
        entry_card_style = entry_card_style[:entry_card_style.index('}') + 1]
        self.assertIn('grid-template-columns: 42px minmax(0, 1fr) auto;', entry_card_style)
        self.assertIn('.account-proxy-entry-icon {', self.styles)
        self.assertNotIn('data-account-proxy-inline-options', picker_entry)
        self.assertNotIn('accountProxyOptionCardsHtml', picker_entry)
        self.assertIn('const originalDialog = returnModal?.querySelector(".console-modal-dialog") || null', picker_open)
        self.assertIn('originalDialog.replaceWith(pickerDialog)', picker_open)
        self.assertIn('pickerDialog.replaceWith(originalDialog)', picker_open)
        self.assertIn('modalRoot = returnModal || document.createElement("div")', picker_open)
        self.assertIn('if (!returnModal)', picker_open)
        self.assertIn("accountProxyPickerPreloadHtml()", picker_open)
        self.assertIn("revealAccountProxyPicker(modalRoot, selectedProxyId", picker_open)
        self.assertIn("attachCountryMenu", picker_open)
        self.assertIn("[data-account-proxy-preload-retry]", picker_open)
        self.assertIn('accountProxyPoolFiltersHtml("modal"', self._function_source("accountProxyPickerReadyHtml"))
        self.assertIn('accountProxyOptionCardsHtml(selectedProxyId, { scope: "modal" })', self._function_source("accountProxyPickerReadyHtml"))
        self.assertIn("async function revealAccountProxyPicker", self.source)
        self.assertIn("data?.purchase_options?.regions", self.source)
        self.assertIn("accountProxyPickerReadyHtml(selectedProxyId)", self.source)
        self.assertIn("if (typeof attachCountryMenu === \"function\") attachCountryMenu()", self.source)
        self.assertIn(".account-proxy-picker-preload {", self.styles)
        self.assertIn("account-proxy-picker-spin", self.styles)
        self.assertIn('pickerDialog.addEventListener("click"', picker_open)
        self.assertIn('pickerDialog.addEventListener("change"', picker_open)
        self.assertIn('event.target.closest("button[data-account-proxy-type]")', picker_open)
        self.assertNotIn('event.target.closest("[data-account-proxy-type]")', picker_open)
        self.assertNotIn('returnModal.hidden = true', picker_open)
        self.assertNotIn('returnModal.hidden = false', picker_open)
        self.assertIn('returnModal.dataset.accountModalPage = "proxy"', picker_open)
        self.assertIn('returnModal.dataset.accountModalPage = "editor"', picker_open)
        self.assertIn('returnModal ? "has-return" : ""', picker_open)
        self.assertIn('data-account-proxy-picker-back', picker_open)
        self.assertIn('renderPersonaProfileEditorBackIcon()', picker_open)
        self.assertIn('event.stopPropagation()', picker_open)
        self.assertNotIn('openAccountProxySelectorPage', self.source)
        self.assertNotIn('renderAccountProxySelectorPage', self.source)
        self.assertIn('.console-modal[hidden]', self.styles)
        self.assertIn('.account-proxy-picker-modal', self.styles)
        self.assertNotIn('.account-proxy-selector-shell', self.styles)
        self.assertIn('white-space: nowrap;', self.styles[self.styles.index('.account-proxy-entry-copy small {'):])

    def test_account_proxy_modal_keeps_filter_interaction_stable_and_content_sized(self):
        picker = self._function_source("openAccountProxyPickerModal")
        shell_start = self.styles.index(".account-proxy-picker-modal {")
        shell_rule = self.styles[shell_start:self.styles.index("}", shell_start) + 1]

        self.assertIn('pickerDialog.addEventListener("input"', picker)
        self.assertIn('pickerDialog.addEventListener("change"', picker)
        self.assertIn('data-account-proxy-picker-cancel', picker)
        self.assertNotIn("min-height: calc(100dvh", shell_rule)
        self.assertIn("max-height: min(760px, calc(100dvh - 32px))", shell_rule)
        self.assertNotIn(".account-proxy-selector-view", self.styles)

    def test_account_create_starts_platform_authorization_and_keeps_browser_editor_for_legacy_accounts(self):
        editor = self._function_source("renderAccountEditorForm")
        identity = self._function_source("renderAccountIdentityFields")
        ...
        bundle_modal = self._function_source("openBundleAccountAuthorizationModal")
        bundle_switch = self._function_source("openBundleAccountSwitchPopup")
        self.assertIn("openBundleAccountAuthorizationModal", create_entry)
        self.assertNotIn("openAccountPoolEditorModal", create_entry)
        self.assertIn("openConsoleModal", bundle_modal)
        self.assertIn("prepareBundleAccountAuthorization", bundle_modal)
        self.assertIn("startBundleAccountAuthorization", bundle_modal)
        self.assertIn("官方授权", bundle_modal)
        self.assertIn('value: "switch-account"', bundle_modal)
        self.assertIn("openBundleAccountSwitchPopup", bundle_modal)
        self.assertIn("showConfirm: editing || accountSwitchReady", bundle_modal)
        self.assertIn("开始授权", bundle_modal)
        self.assertIn("https://www.threads.com/login/", bundle_switch)
        self.assertIn("https://www.instagram.com/accounts/login/", bundle_switch)
        ...
        self.assertNotIn("initialThreadsOauthResult", self.source)
        self.assertNotIn("initialInstagramOauthResult", self.source)
        bundle_result = self._function_source("consumeBundleAuthorizationResult")
        bundle_start = self._function_source("startBundleAccountAuthorization")
        bundle_apply = self._function_source("applyBundleAuthorizationResult")
        bundle_open = self._function_source("openBundleAuthorizationPopup")
        self.assertIn("openBundleAuthorizationPopup", bundle_start)
        self.assertIn("authorizationPopup", bundle_start)
        self.assertIn("authorizationPopup", bundle_open)
        self.assertIn("preparedResult || await prepareBundleAccountAuthorization", bundle_start)
        self.assertNotIn("about:blank", bundle_open)
        self.assertIn("bundleAuthorizationUsesSamePage()", bundle_open)
        self.assertIn("rememberBundleAuthorizationReturnState(platform)", bundle_open)
        self.assertIn("window.location.assign(targetUrl)", bundle_open)
        self.assertIn("consumeBundleAuthorizationReturnState()", bundle_apply)
        self.assertIn("restoreConsoleScrollState(returnState.scroll)", bundle_apply)
        ...
        self.assertIn("state.preferredAccountId = cleanId", account_select)
        self.assertIn("await loadSocial()", bundle_apply)
        google_result = self._function_source("consumeGoogleAccountSessionResult")
        self.assertIn('editing ? "保存修改" : "保存并登录"', modal)
        self.assertIn('createSocialTask("open_login", account.id, payload.persona_id, "socialMsg")', save)
        self.assertIn("openLoginAssistanceView(taskId, account.id)", save)
    ...
    def test_account_proxy_picker_matches_backend_eligibility_and_tracks_real_changes(self):
        ...
        self.assertIn("data-account-proxy-owned-choice", options)
        self.assertIn("const regionLabel = accountProxyCountry(proxy).label", options)
        self.assertIn('class="proxy-market-mini-card-banner"', options)
        self.assertIn('class="proxy-market-mini-title"', options)
        self.assertIn('${esc(regionLabel)}代理 IP', options)
        self.assertIn('${esc(accountProxyCountry(countryCode).label)}静态住宅代理 IP', options)
        self.assertIn('class="proxy-market-compact-fields"', options)
    ...
    def test_account_proxy_picker_uses_the_public_pool_without_custom_proxy_forms(self):
        picker = self._function_source("openAccountProxyPickerModal")
        entry_copy = self._function_source("accountProxyEntryCopy")
        ...
        picker_entry = self._function_source("accountProxyEntryCardHtml")
        proxy_pool = self._function_source("renderProxyPool")

        self.assertIn("revealAccountProxyPicker", picker)
        self.assertIn("accountProxyPickerPreloadHtml", picker)
        self.assertIn("/api/proxy-purchases/options", self._function_source("fetchAccountProxyPickerPool"))
        self.assertIn("accountProxyPoolFiltersHtml", self._function_source("accountProxyPickerReadyHtml"))
        self.assertIn("selectOwnedAccountProxyOption", picker)
        self.assertIn('data-account-proxy-filter="city"', self.source)
        self.assertIn('data-account-proxy-filter-menu="${esc(name)}"', self.source)
        self.assertIn('name: "city"', self.source)
        self.assertIn('name: "sort"', self.source)
        self.assertIn("function accountProxySelectMenuHtml", self.source)
        self.assertIn('class="account-proxy-picker-location-row"', self.source)
        self.assertNotIn('class="account-proxy-picker-action-row"', self.source)
        self.assertIn("选择代理", entry_copy)
        ...
        self.assertIn("data-account-proxy-picker-open", picker_entry)
    ...
    def test_proxy_selection_confirms_free_claim_and_keeps_renewal_in_cards(self):
        claim = self._function_source("purchaseAccountProxySupplierOption")
        ...
        self.assertNotIn("实时购买", cards)
        self.assertNotIn("购买并使用", cards)
        self.assertIn("选择使用", cards)
        self.assertIn('data-account-proxy-renewal-order', cards)
        self.assertIn('"本月免费"', cards)
        self.assertIn('data-account-proxy-supplier-choice', cards)
        self.assertIn('data-account-proxy-purchase-renewal', cards)
        self.assertIn('class="proxy-market-mini-card-banner"', cards)
