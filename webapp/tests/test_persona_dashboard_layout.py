...
class PersonaDashboardLayoutContractTests(unittest.TestCase):
    ...
    def test_dashboard_uses_visible_platform_cards_aligned_with_persona_data_without_legacy_filters_or_binding_form(self):
        ...
        self.assertNotIn('id="personaDashboardPlatformTabs"', dashboard)
        self.assertNotIn("persona-dashboard-top-controls", dashboard)
        ...
        self.assertIn('id="personaDashboardContext"', dashboard)
    ...
    def test_trend_chart_supports_day_month_and_year_aggregation(self):
        ...
        self.assertIn("return keys.map((date) => grouped.get(date)", chart)
        self.assertIn("_hasData: false", chart)
    ...
    def test_trend_aggregation_keeps_real_periods_and_uses_latest_snapshot(self):
        ...
        self.assertEqual(len(aggregated["day"]), 30)
        self.assertEqual(aggregated["day"][0]["date"], "2026-08-02")
        self.assertEqual(aggregated["day"][-1]["date"], "2026-08-31")
        self.assertFalse(aggregated["day"][0]["_hasData"])
        self.assertTrue(aggregated["day"][-1]["_hasData"])
        self.assertEqual(len(aggregated["month"]), 12)
        self.assertEqual(aggregated["month"][0]["date"], "2025-09")
        self.assertEqual(aggregated["month"][-1]["date"], "2026-08")
        self.assertEqual(aggregated["month"][-1]["post_views"], 2620)
        self.assertEqual(len(aggregated["year"]), 5)
        self.assertEqual([row["date"] for row in aggregated["year"]], ["2022", "2023", "2024", "2025", "2026"])
        self.assertEqual(aggregated["year"][-2]["post_views"], 100)
        self.assertEqual(aggregated["year"][-1]["published"], 1)
        self.assertEqual(aggregated["year"][-1]["post_views"], 2620)
        self.assertEqual(aggregated["year"][-1]["likes"], 127)
        self.assertEqual(aggregated["year"][-1]["followers"], 12)
        self.assertEqual(aggregated["year"][-1]["hot_score"], 2700)
    ...
    def test_mobile_account_pool_uses_swipeable_platform_tabs_and_an_empty_account_hint(self):
        ...
        self.assertIn('href="/assets/brands/instagram-glyph-gradient.svg"', self.console_script)
        self.assertNotIn("platformIconRenderSequence", self.console_script)
        self.assertNotIn("instagramOfficialGradient", self.console_script)
        self.assertIn('[data-account-pool-platform="instagram"]:not(.is-active) > strong', self.styles)
        self.assertIn('[data-publish-platform-tab="instagram"]:not(.is-active) > span', self.styles)
        self.assertIn('.google-account-session-platform[data-account-platform="instagram"] .google-account-session-platform-brand > strong', self.styles)
        self.assertIn('-webkit-background-clip: text;', self.styles)
        self.assertNotIn('filter: grayscale(1) brightness(0) invert(1);', self.styles)
        self.assertIn('--instagram-platform-gradient: linear-gradient(135deg, #405de6 0%, #833ab4 30%, #c13584 52%, #e1306c 68%, #f77737 86%, #fcaf45 100%);', self.styles)
        self.assertIn('--instagram-brand-gradient: linear-gradient(120deg, #ffcc00 0%, #fe4a05 24%, #ff0f3f 43%, #ff005f 58%, #fc01d8 78%, #780cff 100%);', self.styles)
        self.assertIn('[data-account-pool-platform="instagram"],', self.styles)
        self.assertIn('[data-persona-content-platform="instagram"],', self.styles)
        self.assertIn('[data-persona-account-platform="instagram"],', self.styles)
        self.assertIn('[data-account-pool-platform="instagram"],', self.styles)
        self.assertIn('[data-matrix-publish-platform-option="instagram"],', self.styles)
        self.assertIn('[data-persona-dashboard-platform-option="instagram"]', self.styles)
        self.assertIn('.publish-platform-picker-tabs button[data-publish-platform-tab="instagram"].is-active', self.styles)
        self.assertIn('.persona-execution-platform-logo[data-persona-content-platform="instagram"].is-current', self.styles)
        self.assertIn('filter: brightness(0) invert(1);', self.styles)
        self.assertNotIn('--instagram-platform-tab-border-gradient:', self.styles)
    ...
    def test_persona_identity_has_a_platform_account_homepage_action(self):
        ...
        self.assertIn('function personaAccountHomepageTarget(', self.console_script)
...
"""
    ...
    def test_persona_image_prompt_uses_text_generation_without_reference_controls(self):
        ...
        self.assertIn("delete state.personaForms[key].images.editImageId;", self.console_script)
        ...
        self.assertNotIn("function togglePersonaImageEditSource", self.console_script)
        self.assertNotIn("data-persona-image-edit=", self.console_script)
        ...
        self.assertNotIn("function personaImageLibraryPromptLabel(item)", self.console_script)
        ...
        self.assertNotIn("data-persona-image-edit=", image_preview)
        self.assertIn("button.disabled = Boolean(busy);", self.console_script)
    ...
    def test_persona_image_delete_confirm_returns_to_profile_image_page(self):
        ...
        confirm_end = self.console_script.index("\nlet consoleScrollPreserveDepth", confirm_start)
        confirm = self.console_script[confirm_start:confirm_end]
        ...
        delete_start = self.console_script.index("async function deletePersonaLibraryImage(")
        delete_end = self.console_script.index("\nasync function refreshPersonaMediaTask", delete_start)
        delete_fn = self.console_script[delete_start:delete_end]

        self.assertIn("stack: stack ?? Boolean($(\"consoleModal\"))", confirm)
        self.assertIn('title: "删除人设图"', editor_handler)
        self.assertIn('return run(() => deletePersonaLibraryImage(imageId), "image")', editor_handler)
        self.assertNotIn("openPersonaProfileEditorModal()", editor_handler.split("data-persona-delete-image")[1])
        self.assertIn('editor?.dataset.modalKey === "persona-profile-editor"', delete_fn)
        self.assertIn('renderPersonaProfileEditorModal(editor.dataset.personaProfileEditorPage || "image")', delete_fn)
    ...
    def test_persona_profile_editor_emphasizes_name_and_opens_image_lightbox(self):
        ...
        self.assertNotIn("const imageEdit = event.target.closest(\"[data-persona-image-edit]\");", editor_handler)
        ...
        self.assertNotIn(".persona-image-library-edit-button {", self.styles)
        self.assertIn(".persona-image-library-card.is-selected {", self.styles)
        self.assertNotIn(".persona-image-library-card.is-modify-source", self.styles)
        self.assertIn("var(--media-edit-flow-gradient)", self.styles)
        self.assertIn("const selected = selectedCardId === item.id;", self.console_script)
    ...
    def test_persona_account_settings_precedes_history_and_uses_inline_change_actions(self):
        ...
        self.assertIn('class="account-card-action persona-account-card-action persona-account-card-unbind"', actions)
        ...
        self.assertIn('class="account-card-action persona-account-card-action persona-account-card-change"', actions)
