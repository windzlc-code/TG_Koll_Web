...
def _verify_tweet_bot_token(token: str) -> dict[str, Any]:
    """Verify a token and return an actionable, non-sensitive admin error."""
    try:
        ...
        # Log only the exception type: urllib/aiohttp errors can include a URL
        # or request data, neither of which belongs in the admin log.
        logger.warning("Telegram tweet Bot token verification failed: %s", type(exc).__name__)
        if "conflict" in reason or "409" in reason:
            raise HTTPException(
                status_code=409,
                detail="推文 Bot Token 正在被其他轮询实例占用（Telegram 409），请先停止旧 Bot 实例后重试。",
            ) from exc
        if any(marker in reason for marker in ("not found", "unauthorized", "invalid token", "401", "404")):
            raise HTTPException(
                status_code=400,
                detail="推文 Bot Token 无效或已撤销，请从 @BotFather 重新生成后再保存。",
            ) from exc
        raise HTTPException(
            status_code=502,
            detail="暂时无法连接 Telegram API，Token 尚未保存；请检查服务器网络或代理后重试。",
        ) from exc
...
def _tweet_authorization_error_page(message: str, *, status_code: int = 410) -> HTMLResponse:
    ...
    content = render_telegram_authorization_error_page(
        message,
        page_title="Telegram 推文工作台授权",
        workbench_label="推文工作台",
...
def load_tweet_tg_settings(get_runtime: GetRuntime) -> dict[str, Any]:
    ...
    return {
        ...
        "bot_starting": bool(status.get("starting")),
...
async def _run_bot(get_runtime: GetRuntime) -> None:
    while not _BOT_STOP.is_set():
        _update_bot_status({"starting": True, "last_error": "", "updated_at": time.time()})
        runtime = get_runtime() or {}
        token = str(runtime.get("telegram_tweet_bot_token") or "").strip()
        enabled = bool(runtime.get("telegram_tweet_bot_enabled"))
        video_token = str(runtime.get("telegram_bot_token") or "").strip()
        if not token or not enabled:
            _update_bot_status({"running": False, "starting": False, "last_error": "未启用或未配置 Token", "updated_at": time.time()})
            return
        if bool(runtime.get("telegram_bot_enabled")) and video_token and secrets.compare_digest(token, video_token):
            _update_bot_status({"running": False, "starting": False, "last_error": "推文 Bot 与视频 Bot Token 冲突，已拒绝启动推文 Bot", "updated_at": time.time()})
            return
        if _BOT_OPS is None:
            _update_bot_status({"running": False, "starting": False, "last_error": "推文 Bot 业务适配器尚未初始化", "updated_at": time.time()})
            return
        if not _acquire_or_renew_bot_lease(token):
            _update_bot_status({"running": False, "starting": False, "last_error": "另一个推文 Bot 实例正在轮询，本实例保持待命", "updated_at": time.time()})
        ...
        try:
            ...
            _update_bot_status({"running": False, "starting": False, "last_error": str(exc)[:400], "updated_at": time.time()})
            await asyncio.sleep(5)
        finally:
            with contextlib.suppress(Exception):
                _release_bot_lease(token)
            _update_bot_status({"running": False, "starting": False, "updated_at": time.time()})
...
def start_tweet_telegram_bot_worker(get_runtime: GetRuntime) -> None:
    ...
    with _BOT_LOCK:
        if _BOT_THREAD and _BOT_THREAD.is_alive():
            return
        _BOT_STOP.clear()
        _update_bot_status({"running": False, "starting": True, "last_error": "", "updated_at": time.time()})
...
def inject_tweet_telegram_admin(
    ...
    *,
    ...
    def test_env(payload: TweetTgEnvPayload, _user: dict[str, Any] = Depends(require_admin)):
        ...
        return {"ok": True, **_verify_tweet_bot_token(token)}
    ...
    def open_workbench(ticket: str = ""):
        ...
        response = HTMLResponse(
            render_telegram_authorization_page(
                ticket=ticket,
                workbench_label="推文工作台",
                page_title="绑定 Telegram 推文工作台",
                login_path="/console-login.html",
                return_path="/telegram/tweet/open",
                context_key="vecto-telegram-tweet-login-context",
                context_flag="telegram_tweet",
                exchange_path="/telegram/tweet/exchange",
                fallback_target=WEBAPP_TARGET,
            ),
            status_code=200,
        )
        response.headers["Cache-Control"] = "no-store, max-age=0"
