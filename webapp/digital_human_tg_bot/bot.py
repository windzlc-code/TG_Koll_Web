...
# Keep the compact icon-first controls on the visible Telegram keyboard. The
# plain/traditional labels remain accepted so keyboards sent before this
...
# The account page intentionally stays flat.  Provider choice (VECTO
# password login or Google OAuth) belongs to the HTTPS page opened by the
# first button, not to another Telegram callback page.
VIDEO_LOGOUT_BUTTON = "🚪 退出登录"
VIDEO_WEB_ACCOUNT_BUTTON = "🌐 网页登录/切换"
VIDEO_GOOGLE_ACCOUNT_BUTTON = "Google 官方授权登录/切换"
VIDEO_LOGIN_CANCEL_BUTTON = "取消当前登录步骤"
VIDEO_LOGOUT_CONFIRM_BUTTON = "确认退出 VECTO 账号"
VIDEO_LOGOUT_CANCEL_BUTTON = "取消退出账号"
ACCOUNT_BACK_BUTTON = "返回工作台"
VIDEO_ACCOUNT_SESSION_BUTTON = "VECTO 网页账号（登录/退出）"
VIDEO_ACCOUNT_BACK_BUTTON = "返回账号管理"
# Keep labels that may already be visible in an older Telegram keyboard
# accepted as text input.  New keyboards use the plain official brand names
# above, without invented provider icons.
VIDEO_LOGIN_LEGACY_BUTTON = "🔐 登录 VECTO 账号"
VIDEO_SWITCH_LEGACY_BUTTON = "🔄 切换 VECTO 账号"
VIDEO_LOGOUT_LEGACY_BUTTON = "🚪 退出 VECTO 账号"
VIDEO_ACCOUNT_SESSION_LEGACY_BUTTON = "🔐 VECTO 网页账号（登录/退出）"
...
def build_dispatcher(
    ...
    *,
    ...
    web_login_url: Callable[[int], str] | None = None,
    chat_authorized: Callable[[int], bool] | None = None,
) -> Dispatcher:
    ...
    video_login_pending: dict[int, dict[str, Any]] = {}
    ...
    def _is_chat_authorized(chat_id: int) -> bool:
        ...
        if chat_authorized is not None:
            try:
                ...
                logger.exception("Failed to resolve live video Bot authorization")
        return bool(service.is_chat_authorized(int(chat_id)))
    ...
    def _account_management_keyboard(
        *,
        ...
        if clean_chat_id > 0 and _is_chat_authorized(clean_chat_id):
            login_button = _video_web_login_button(
                clean_chat_id,
                button_text=VIDEO_WEB_ACCOUNT_BUTTON,
            )
            rows.append([
                login_button
    ...
    def _video_provider_url(login_url: str, provider: str | None) -> str:
        provider_name = str(provider or "").strip().lower()
        if provider_name not in {"google", "password"}:
            # The flat Telegram entry deliberately keeps the original ticket
            # URL.  The external page itself exposes both VECTO and Google
            # login branches.
            return str(login_url or "")
        ...
        *,
        ...
        return InlineKeyboardButton(
            text=(
                ...
                or ("打开 Google 官方授权" if provider == "google" else "打开 VECTO 网页授权")
            ),
    ...
    def _account_keyboard(
        *,
        ...
        include_logout: bool = False,
        back_text: str = VIDEO_ACCOUNT_BACK_BUTTON,
        chat_id: int | None = None,
    ) -> InlineKeyboardMarkup:
        rows: list[list[InlineKeyboardButton]] = []
        if include_login:
    ...
    def _video_login_keyboard(placeholder: str) -> InlineKeyboardMarkup:
        ...
        return InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(
                text=VIDEO_LOGIN_CANCEL_BUTTON,
                callback_data="tv:login_cancel",
            )],
            [InlineKeyboardButton(
                text=VIDEO_ACCOUNT_BACK_BUTTON,
                callback_data="tv:accountmenu",
            )],
        ])
    ...
    async def ensure_authorized(message: Message) -> bool:
        ...
        if _is_chat_authorized(chat_id):
            ...
            return True
        await message.answer(
            "当前 Telegram 账号尚未绑定 VECTO 视频工作台，请在“账号管理”中打开网页授权入口完成绑定。",
            reply_markup=_account_keyboard(
                include_login=True,
                include_logout=False,
                chat_id=chat_id,
    ...
    async def _account_status_payload(chat_id: int) -> tuple[str, InlineKeyboardMarkup]:
        ...
        if not authorized:
            return (
                "账号管理\n\n"
                "当前 Telegram 账号已被管理员停用。\n"
                "请联系管理员重新启用后，再发送 /start。",
    ...
    async def _send_account_status(message: Message) -> None:
        ...
        if chat is None or str(getattr(chat, "type", "") or "") != "private" or actor is None:
            ...
            return
        page_text, markup = await _account_status_payload(int(chat.id))
    ...
    async def _send_video_account_session_status(message: Message) -> None:
        ...
        if chat is None or str(getattr(chat, "type", "") or "") != "private" or actor is None:
            await message.answer("视频工作台账号管理只支持与 Bot 私聊使用。")
            return
        page_text, markup = await _video_account_session_payload(int(chat.id))
        await message.answer(page_text, reply_markup=markup)

    async def _start_video_account_login(
        message: Message,
        state: FSMContext,
        *,
        ...
        if (
            ...
            await respond("网页登录授权只支持与视频 Bot 私聊使用。")
            return
        ...
        if not login_url:
            ...
            return
        # ``provider`` remains in the signature for old callback payloads, but
        # provider selection is deliberately moved to the external webpage.
        # Telegram should expose one browser entry, not a second provider menu.
        _ = provider
        step_text = (
            "🌐 网页登录/切换\n\n"
            "请点击下方按钮打开 VECTO 官方网页。\n"
            "网页中可选择 VECTO 账号登录或 Google 官方授权，完成后会自动返回并绑定视频工作台。\n"
            "Telegram 不接收或保存账号密码、验证码。"
        )
        await respond(
            step_text,
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [_video_web_login_button(
                    chat_id,
                    login_url=login_url,
                    button_text=VIDEO_WEB_ACCOUNT_BUTTON,
                )],
                [InlineKeyboardButton(text=VIDEO_LOGIN_CANCEL_BUTTON, callback_data="tv:login_cancel")],
                [InlineKeyboardButton(text="↩️ 返回账号管理", callback_data="tv:accountmenu")],
            ]),
        )

    async def _show_video_logout_step(
        message: Message,
        *,
        edit_message: bool = False,
    ) -> None:
        ...
        member = _load_bound_member(chat_id)
        username = str(_member_value(member, "web_username", "") or "").strip() or "当前 VECTO 账号"
        text = (
            "退出 VECTO 账号步骤\n\n"
            f"步骤 1/2：确认退出 {username}。\n"
            "步骤 2/2：确认后会撤销此 Telegram 账号关联的网页会话，视频工作台将立即锁定。\n\n"
            "如只是切换设备或重新登录，请选择取消退出。"
        )
        markup = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=f"✅ {VIDEO_LOGOUT_CONFIRM_BUTTON}", callback_data="tv:aclogout_confirm")],
            [InlineKeyboardButton(text=f"↩️ {VIDEO_LOGOUT_CANCEL_BUTTON}", callback_data="tv:accountmenu")],
        ])
        if edit_message and getattr(message, "edit_text", None) is not None:
    ...
    async def _handle_video_account_login_input(message: Message, state: FSMContext) -> None:
        ...
        if clean_text in {VIDEO_LOGIN_CANCEL_BUTTON, VIDEO_ACCOUNT_BACK_BUTTON}:
            ...
            if clean_text == VIDEO_LOGIN_CANCEL_BUTTON:
                await message.answer("已取消视频工作台登录。", reply_markup=_account_keyboard(chat_id=chat_id))
            else:
    ...
    async def _perform_video_account_logout(chat_id: int) -> tuple[str, bool]:
        """Revoke the shared VECTO web session without changing Bot menus."""
        if logout_member is None:
            return "❌ 退出服务尚未就绪，请稍后重试。", False
        try:
            result = await asyncio.to_thread(logout_member, int(chat_id))
        except Exception:
            logger.exception("Failed to log out video Bot account")
            return "❌ 退出账号失败，请稍后重试。", False
        if isinstance(result, dict) and result.get("legacy_authorized"):
            return "⚠️ 当前是兼容登记记录，不能直接退出网页账号；请先登录 VECTO 账号后再管理会话。", False
        service.upsert_member(
            chat_id=int(chat_id),
            label=f"TG-{int(chat_id)}",
            enabled=True,
            notify_busy=True,
            notify_available=True,
        )
        _clear_video_login(int(chat_id))
        return (
            "✅ 已退出登录；网页会话已撤销，视频工作台已锁定。\n"
            "如需继续，请从“VECTO 网页账号”重新登录。",
            True,
        )

    @router.callback_query(F.data.startswith("tv:"))
    async def video_account_callback(query: CallbackQuery, state: FSMContext) -> None:
        """Keep every video account sub-step in one editable callback page."""
        ...
        if (
            ...
            await message.edit_text("视频工作台账号管理只支持与 Bot 私聊使用。")
            return
        ...
        try:
            if action == "menu":
                ...
                _clear_video_login(chat_id)
                await state.clear()
                page_text, markup = await _account_status_payload(chat_id)
                await message.edit_text(page_text, reply_markup=markup)
            elif action == "vectosession":
                _clear_video_login(chat_id)
                await state.clear()
                page_text, markup = await _video_account_session_payload(chat_id)
                await message.edit_text(page_text, reply_markup=markup)
            elif action == "chatlogin":
                await _start_video_account_login(
                    message,
                    state,
                    from_user=actor,
                    edit_message=True,
                )
            elif action == "login_web":
                await _start_video_account_login(
                    message,
                    state,
                    from_user=actor,
            ...
            elif action == "logout_step":
                _clear_video_login(chat_id)
                await state.clear()
                await _show_video_logout_step(message, edit_message=True)
            elif action == "aclogout":
                _clear_video_login(chat_id)
                await state.clear()
                await _show_video_logout_step(message, edit_message=True)
            elif action == "aclogout_confirm":
                _clear_video_login(chat_id)
                await state.clear()
                result_text, _ = await _perform_video_account_logout(chat_id)
                page_text, markup = await _video_account_session_payload(chat_id)
                await message.edit_text(
                    result_text + "\n\n" + page_text,
                    reply_markup=markup,
            ...
            logger.exception("Video account callback failed: %s", action)
            await message.edit_text(
                "❌ 账号操作暂时失败，请稍后重试。",
                reply_markup=_account_management_keyboard(),
            )
    ...
    @router.message(F.text.in_(VIDEO_ACCOUNT_SESSION_TEXTS))
    async def video_account_session(message: Message, state: FSMContext) -> None:
        await state.clear()
        if not _is_chat_authorized(int(getattr(getattr(message, "chat", None), "id", 0) or 0)):
            await message.answer(
                VIDEO_ACCESS_DENIED_TEXT,
            ...
            return
        await _send_video_account_session_status(message)

    @router.message(F.text == VIDEO_ACCOUNT_BACK_BUTTON)
    async def video_account_session_back(message: Message, state: FSMContext) -> None:
        await state.clear()
        await _send_account_status(message)
    ...
    @router.message(F.text.in_(VIDEO_LOGIN_TEXTS))
    ...
    async def video_account_login_cancel(message: Message, state: FSMContext) -> None:
        ...
        await message.answer(
            "已取消视频工作台登录。",
            reply_markup=_account_keyboard(chat_id=int(getattr(getattr(message, "chat", None), "id", 0) or 0)),
        )
    ...
    @router.message(F.text.in_(VIDEO_LOGOUT_TEXTS))
