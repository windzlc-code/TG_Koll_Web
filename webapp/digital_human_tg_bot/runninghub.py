...
class RunningHubClient:
    ...
    def submit_ai_app_task(
        ...
        *,
        ...
        self._check_cancelled()
        webapp_id_text = str(webapp_id or "").strip()
        requested_instance_type = str(instance_type or "").strip()
        resolved_instance_type = requested_instance_type
        if not resolved_instance_type and webapp_id_text in PLUS_WORKFLOW_IDS:
            resolved_instance_type = "plus"
        use_v2_api = bool(resolved_instance_type and resolved_instance_type.lower() not in {"default", "normal"})
        if use_v2_api:
            submit_url = f"{self.base_url}/openapi/v2/run/ai-app/{webapp_id_text}"
            payload = {
            ...
            submit_url = f"{self.base_url}/task/openapi/ai-app/run"
            payload = {
                "webappId": webapp_id_text,
                "apiKey": self.api_key,
                "nodeInfoList": node_info_list,
            }
