...
- 固定键盘只保留四个跨流程入口：我的人设、任务中心、账号与浏览器、停止当前任务。创作、内容和发布不再拆成独立总控入口。
- 我的人设：页面顶部提供矩阵发布和新建人设，下面分页列出人设；选择人设后进入统一详情页，生成、热点、推文内容、发布、设置和账号状态均从该详情页继续。
...
- 发布：单篇发布按“推文 -> Threads/Instagram 账号 -> 立即/定时 -> 最终确认”执行；矩阵发布按“勾选人设 -> 草稿/收藏 -> 平台 -> 汇总确认”执行，确认后才进入既有队列。当前平台批次上限为每个人设 1 篇，因此不显示无效的数量选项。
- 任务：先选择进行中、失败或全部任务，再进入任务详情；支持取消，社交发布失败后可重试，生成失败后可重新生成。“停止当前任务”会同时取消未提交输入与当前运行/排队任务，不会误删未来定时任务。
