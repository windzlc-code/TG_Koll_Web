...
const labels = {
  ...
  "zh-Hant": {
    ...
    noPools: "尚無客戶池", choosePool: "選擇一個客戶池查看成員", noMembers: "該客戶池尚無成員", noPlatformMembers: "該平台暫無客戶", member: "客戶", platform: "平台", platformFilter: "平台", stage: "階段", score: "評分", source: "來源", tags: "標籤", handle: "帳號", viewMember: "查看", memberDetail: "客戶資料", createTask: "建立任務", mixStages: "跟進階段", customerCount: "位客戶", editPool: "編輯客戶池", poolName: "客戶池名稱", poolTags: "標籤（逗號或換行分隔）", savePool: "儲存名稱與標籤", deduplicate: "檢查並移除重複成員", deduplicated: "已完成成員去重", removed: "移除重複", opcHistory: "OPC 歷史客戶", opcHint: "先預覽篩選結果，再匯入為新的客戶池。", searchHistory: "搜尋帳號、內容或關鍵詞", keywords: "關鍵詞（逗號分隔）", allPlatforms: "全部平台", allContacts: "全部狀態", newContact: "全新名單", contacted: "已觸達", failed: "失敗重試", preview: "預覽匹配客戶", previewing: "正在查詢…", matched: "匹配客戶", excludeExisting: "排除已在客戶池中的帳號", excludeInteracted: "排除已有互動記錄", importPool: "匯入為客戶池", importing: "正在匯入…", imported: "歷史客戶已匯入",
...
export function PoolsView({ language, onCreate }: { language: Language; onCreate?: () => void }) {
  ...
  useEffect(() => { if (poolOpen && selectedId) void loadPool(selectedId); }, [loadPool, selectedId, poolOpen]);
  ...
  return <section className="crm-panel crm-business-panel" aria-busy={state === "loading" || detailState === "loading"}>
    ...
    {state === "ready" && pools.length > 0 && <nav className="crm-pool-list crm-master-list" aria-label={t.pools}>{pools.map((pool) => { const count = Number(pool.lead_count || pool.leadCount || 0); const id = idOf(pool); return <button className={selectedId === id && poolOpen ? "is-active" : ""} type="button" key={id} onClick={() => { setSelectedId(id); setPoolOpen(true); }}><strong>{textOf(pool.name)}</strong><small>{count > 0 ? `${count} ${t.customerCount}` : textOf(pool.description, t.choosePool)}</small></button>; })}</nav>}
    {poolOpen && <ConsoleModal title={textOf(detail?.name, t.poolDetails)} labelledBy="crm-pool-window-title" onClose={() => { setPoolOpen(false); setInspecting(null); }} wide>
