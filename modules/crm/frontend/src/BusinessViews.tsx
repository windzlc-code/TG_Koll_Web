...
import { cronFriendly, humanText, isTechnicalId, metricLabel, mixFromValues, mixParts, mixTone, poolInsightCards, readableTags, workflowLabel, type MixPart } from "./present";
...
export function PoolsView({ language }: { language: Language }) {
  ...
  return <section className="crm-panel crm-business-panel" aria-busy={state === "loading" || detailState === "loading"}>
    ...
    {state === "ready" && pools.length > 0 && <div className="crm-split-workspace">
      <nav className="crm-master-list" aria-label={t.pools}>{pools.map((pool) => { const count = Number(pool.lead_count || pool.leadCount || 0); return <button className={selectedId === idOf(pool) ? "is-active" : ""} type="button" key={idOf(pool)} onClick={() => setSelectedId(idOf(pool))}><strong>{textOf(pool.name)}</strong><small>{count > 0 ? `${count} ${t.customerCount}` : textOf(pool.description, t.choosePool)}</small></button>; })}</nav>
      <div className="crm-detail-pane">
        ...
        {detailState === "ready" && detail && <>
          <header className="crm-detail-heading"><div><span className="crm-kicker">{t.poolDetails}</span><h3>{textOf(detail.name)}</h3><p>{textOf(detail.description, t.choosePool)}</p></div>{tags.length > 0 && <div className="crm-chip-row">{tags.map((tag) => <span key={tag}>{tag}</span>)}</div>}</header>
          ...
          {!members.length ? <p className="crm-quiet-empty">{t.noMembers}</p> : <div className="crm-member-grid">{members.map((member, index) => { const lead = objectOf(member.lead || member.profile); const name = textOf(member.display_name || lead.display_name || member.username || lead.username, t.member); const handle = textOf(member.username || lead.username, ""); const stage = String(member.stage || lead.stage || member.status || ""); return <article className="crm-member-card" key={String(member.lead_id || member.id || index)}><strong>{name}</strong>{handle && handle !== name ? <small>@{handle.replace(/^@/, "")}</small> : null}<footer><PlatformChip platform={member.platform || lead.platform} />{stage ? <span className="crm-chip">{metricLabel(stage, language)}</span> : null}</footer></article>; })}</div>}
