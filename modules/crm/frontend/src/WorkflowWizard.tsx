    ...
    <div className="crm-modal crm-workflow-wizard" ref={dialog} role="dialog" aria-modal="true" aria-labelledby="crmWorkflowTitle">
      ...
      <div className="crm-wizard-body">
        ...
        {((view === "collect" && step === 3) || (view === "relationships" && step === 3) || (view !== "collect" && view !== "relationships" && step === 4)) && <section className="crm-confirmation-summary crm-wizard-confirmation">
          ...
          <dl><div><dt>{t.action}</dt><dd>{view === "public" ? messages.operationLabels[publicAction] : messages.views[view][0]}</dd></div><div><dt>{t.account}</dt><dd>{view === "collect" ? collectActions().map((action) => accounts.find((item) => String(item.id) === action.account_id)?.display_name || action.account_id).filter((name, index, all) => name && all.indexOf(name) === index).join("、") || messages.notSelected : selectedAccount?.display_name || selectedAccount?.username || messages.notSelected}</dd></div><div><dt>{t.targetRange}</dt><dd>{view === "collect" ? `${collectMode} · ${collectActions().length}` : t.selected(effectiveRecipients.length)}</dd></div>{view !== "collect" && view !== "relationships" && <div><dt>{t.execution}</dt><dd>{t[execution]}{execution === "schedule" ? ` · ${new Date(scheduleAt).toLocaleString(language)}` : ""}</dd></div>}{templateId && <div><dt>{t.template}</dt><dd>{String(templates.find((row) => rowId(row) === templateId)?.name || templateId)} · {arrayOf(templates.find((row) => rowId(row) === templateId)?.media_ids).length} {language === "zh-Hant" ? "個附件" : "个附件"}</dd></div>}<div><dt>{t.billing}</dt><dd>{isWrite ? t.billingWrite : t.billingRead}</dd></div></dl>
