...
` : ""}
${todayNews ? `
## 可選新聞參考（不是創作任務，不是必用素材）
以下是本次生成前即時取得、並以人設主題查詢的外部參考。它只能提供可能的背景與切角，不能取代本次使用者要求、人設內核、人設記憶或使用者選定的關鍵詞：
- 優先順序固定為：本次使用者要求 > 人設內核與既有記憶 > 使用者選定方向 > 地區與語言規則 > 新聞參考
- 只有與本次使用者要求或人設核心高度相關時才使用；不相關就忽略，禁止為了追熱點而偏離人設
- 即使相關也可以完全不用；不得因為存在新聞參考就改變原定主題、立場、人物設定或故事進度
- 不要寫成新聞摘要、標題改寫或條列報導，要轉化為人設本人的觀察、經驗、判斷或提問
- 不得捏造情報中沒有的日期、數字、政策結論與人物說法；不確定的內容改成一般性觀察
- 不要逐字複製新聞標題或內文，也不要聲稱已閱讀未提供完整內容的文章
